 ...
 * Export-/Import-/Rollback-Dauer + Speicher-Peak für eine Größenklasse.
 *
...
class PerformanceBaselineTest extends FunctionalTestCase
{
    ...
    /** Live-Seiten mit Herkunfts-Mapping (= importierte Seiten). */
    private function importedPageCount(): int
    {
        $qb = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('pages');
        $qb->getRestrictions()->removeAll();
        return (int)$qb->count('uid')->from('pages')
            ->where(
                $qb->expr()->gt('tx_impexpnl_remote_uid', $qb->createNamedParameter(0, Connection::PARAM_INT)),
                $qb->expr()->eq('deleted', $qb->createNamedParameter(0, Connection::PARAM_INT))
            )
            ->executeQuery()->fetchOne();
    }

    private function newRootUid(): ?int
    {
        $qb = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('pages');
        $qb->getRestrictions()->removeAll();
        $v = $qb->select('uid')->from('pages')
            ->where($qb->expr()->eq('tx_impexpnl_remote_uid', $qb->createNamedParameter(1, Connection::PARAM_INT)))
            ->executeQuery()->fetchOne();
    ...
    public function measuresBaselineForSizeClass(): void
    {
        ...
        self::assertSame($pages, $this->importedPageCount(), 'Nicht alle Seiten importiert');

        $rootNew = $this->newRootUid();
        ...
        self::assertSame(0, $this->importedPageCount(), 'Rollback hat nicht aufgeräumt');
